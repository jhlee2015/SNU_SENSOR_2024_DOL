...
class SUPMEA:
    ...
    def readthread(self):  # 데이터 받는 함수
        ...
        if self.client.connect():
            try:
                while True:
                    serial_logger.info("Kisan Sensor press Request!")
                    ...
                    if read_result.isError():
                        ...
                        #modbus 통신 테스트용
                        #mclient.modbus_tcp_client.write_modbus_double(serial_logger, self.tcp_client, 5, round(987.654, 2))
                    else:
                        ...
                        mclient.modbus_tcp_client.write_modbus_float(serial_logger, self.tcp_client, RTU_ADDR_CH_1,
                                                                     round(cal_val_bar, 2))

                        cal_val = (values[1] / 65535.0) * 20.0  # [1] 1채널
                        serial_logger.info(f"{cal_val:.3f} mA")
                        cal_val_bar = up_util.UTIL.current_to_bar(cal_val)
                        mclient.modbus_tcp_client.write_modbus_float(serial_logger, self.tcp_client, RTU_ADDR_CH_2,
                                                                     round(cal_val_bar, 2))

                        cal_val = (values[2] / 65535.0) * 20.0  # [2] 2채널
                        serial_logger.info(f"{cal_val:.3f} mA")
                        cal_val_bar = up_util.UTIL.current_to_bar(cal_val)
                        mclient.modbus_tcp_client.write_modbus_float(serial_logger, self.tcp_client, RTU_ADDR_CH_3,
                                                                     round(cal_val_bar, 2))

                        cal_val = (values[3] / 65535.0) * 20.0  # [3] 3채널
                        serial_logger.info(f"{cal_val:.3f} mA")
                        cal_val_bar = up_util.UTIL.current_to_bar(cal_val)
                        mclient.modbus_tcp_client.write_modbus_float(serial_logger, self.tcp_client, RTU_ADDR_CH_4,
                                                                     round(cal_val_bar, 2))

                        cal_val = (values[4] / 65535.0) * 20.0  # [4] 4채널
                        serial_logger.info(f"{cal_val:.3f} mA")
                        cal_val_bar = up_util.UTIL.current_to_bar(cal_val)
                        mclient.modbus_tcp_client.write_modbus_float(serial_logger, self.tcp_client, RTU_ADDR_CH_5,
                                                                     round(cal_val_bar, 2))
                    #client.close()
                    time.sleep(5)
...
if __name__ == '__main__':
    ...
    while True:
        serial_logger.info('Han Cnu Kisan press Start')
        try:
            sup = SUPMEA()
